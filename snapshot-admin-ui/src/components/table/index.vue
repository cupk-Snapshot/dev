    ...
    >
      ...
      >
        <template v-slot="scope">
          <template v-if="(item.type === 'image')&&(item.key !== undefined)">
            <el-image
          ...
          <template v-else-if="item.key === 'isPublic'">
            <el-switch
          ...
          <el-dialog v-model="dialogFormVisible" :modal="false" title="审核">
            <div class="flex">
              ...
              <el-radio-group v-model="scope.row.pass">
                ...
                <el-radio :label="'等待审核'">等待审核</el-radio>
            ...
            <div class="flex">
              ...
              <el-radio-group v-model="scope.row.point">
                ...
                <el-radio :label="10">10积分</el-radio>
                <el-radio :label="20">20积分</el-radio>
                <el-radio :label="25">25积分</el-radio>
                <el-radio :label="30">30积分</el-radio>
              </el-radio-group>
            </div>
            <template #footer>
              <span class="dialog-footer">
                <el-button @click="dialogFormVisible = false">取消</el-button>
                <el-button type="primary" @click="onSubmit(scope.row.pass,scope.row.point,scope.row.userId,scope.row.acceptId)">确定</el-button>
              </span>
            </template>
          </el-dialog>
...
//更改公开状态
// const changePublicStatus = ((data: number) => {
//   console.log("==============================")
//   console.log(data)
//   // acceptApi().update(props.data.)
// });


//提交审核
const onSubmit = (status: string, point: number, userId: number, acceptId: number) => {
  let examineId = Session.get('user').userId;
  let examineName = Session.get('user').name;
  let data = {
    'status': status,
    'point': point,
    'userId': userId,
  ...
  acceptApi().examine(data).then(res => {
    console.log(res)
